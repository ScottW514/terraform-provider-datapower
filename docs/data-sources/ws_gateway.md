...
- `app_domain` (String) The name of the application domain the object belongs to
- `auto_create_sources` (Boolean) The front side ports that accept connections must have configured Front Side Protocol Source Handlers. If no Front Side Protocol Handler is configured and auto-create is enabled, a default Protocol Handler will be created. Currently only auto-creates HTTP sources.
