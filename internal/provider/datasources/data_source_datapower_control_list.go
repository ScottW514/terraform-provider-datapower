...
type ControlListList struct {
	AppDomain types.String `tfsdk:"app_domain"`
	Result    types.List   `tfsdk:"result"`
}

var (
	_ datasource.DataSource              = &ControlListDataSource{}
	_ datasource.DataSourceWithConfigure = &ControlListDataSource{}
)

func NewControlListDataSource() datasource.DataSource {
	return &ControlListDataSource{}
}

type ControlListDataSource struct {
	pData *tfutils.ProviderData
}

func (d *ControlListDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
...
func (d *ControlListDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, _ *datasource.ConfigureResponse) {
...
func (d *ControlListDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	d.pData.Mu.Lock()
	defer d.pData.Mu.Unlock()

	var data ControlListList
	resp.Diagnostics.Append(req.Config.Get(ctx, &data)...)
	if resp.Diagnostics.HasError() {
		return
	}
	o := models.ControlList{
	...
	l := []models.ControlList{}
	if value := res.Get(`ControlList`); value.Exists() {
		for _, v := range value.Array() {
			item := models.ControlList{}
			item.FromBody(ctx, "", v)
			if !item.IsNull() {
				l = append(l, item)
			}
	...
	if len(l) > 0 {
		...
		data.Result, diag = types.ListValueFrom(ctx, types.ObjectType{AttrTypes: models.ControlListObjectType}, l)
		resp.Diagnostics.Append(diag...)
		if resp.Diagnostics.HasError() {
			return
		}
	} else {
		data.Result = types.ListNull(types.ObjectType{AttrTypes: models.ControlListObjectType})
	}
