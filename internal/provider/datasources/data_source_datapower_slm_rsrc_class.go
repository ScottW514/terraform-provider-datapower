...
var (
	_ datasource.DataSource              = &SLMRsrcClassDataSource{}
	_ datasource.DataSourceWithConfigure = &SLMRsrcClassDataSource{}
)

func NewSLMRsrcClassDataSource() datasource.DataSource {
	return &SLMRsrcClassDataSource{}
}

type SLMRsrcClassDataSource struct {
	pData *tfutils.ProviderData
}

func (d *SLMRsrcClassDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
				...
				NestedObject: schema.NestedAttributeObject{
					Attributes: map[string]schema.Attribute{
						...
						"rsrc_value": schema.ListAttribute{
							MarkdownDescription: "Resource value",
						...
						"subscription": schema.StringAttribute{
							MarkdownDescription: "UDDI subscription (deprecated)",
							Computed:            true,
						},
...
func (d *SLMRsrcClassDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, _ *datasource.ConfigureResponse) {
...
func (d *SLMRsrcClassDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	d.pData.Mu.Lock()
	defer d.pData.Mu.Unlock()

	var data SLMRsrcClassList
	resp.Diagnostics.Append(req.Config.Get(ctx, &data)...)
	if resp.Diagnostics.HasError() {
		return
	}
	o := models.SLMRsrcClass{
	...
	l := []models.SLMRsrcClass{}
	if resFound {
		if value := res.Get(`SLMRsrcClass`); value.Exists() {
			for _, v := range value.Array() {
				item := models.SLMRsrcClass{}
				item.FromBody(ctx, "", v)
				if !item.IsNull() {
					l = append(l, item)
				}
	...
	if len(l) > 0 {
		...
		data.Result, diag = types.ListValueFrom(ctx, types.ObjectType{AttrTypes: models.SLMRsrcClassObjectType}, l)
		resp.Diagnostics.Append(diag...)
		if resp.Diagnostics.HasError() {
			return
		}
	} else {
		data.Result = types.ListNull(types.ObjectType{AttrTypes: models.SLMRsrcClassObjectType})
	}
