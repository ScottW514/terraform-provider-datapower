...
type SFTPFilePollerSourceProtocolHandlerList struct {
	AppDomain types.String `tfsdk:"app_domain"`
	Result    types.List   `tfsdk:"result"`
}

var (
	_ datasource.DataSource              = &SFTPFilePollerSourceProtocolHandlerDataSource{}
	_ datasource.DataSourceWithConfigure = &SFTPFilePollerSourceProtocolHandlerDataSource{}
)

func NewSFTPFilePollerSourceProtocolHandlerDataSource() datasource.DataSource {
	return &SFTPFilePollerSourceProtocolHandlerDataSource{}
}

type SFTPFilePollerSourceProtocolHandlerDataSource struct {
	pData *tfutils.ProviderData
}

func (d *SFTPFilePollerSourceProtocolHandlerDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_sftp_file_poller_source_protocol_handler"
}

func (d *SFTPFilePollerSourceProtocolHandlerDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
				...
				NestedObject: schema.NestedAttributeObject{
					Attributes: map[string]schema.Attribute{
						...
						"ssh_client_connection": schema.StringAttribute{
							MarkdownDescription: "Specifies the name of the SSH client profile which describes how SSH connections are made from the SSH client to an SSH server.",
							Computed:            true,
						},
...
func (d *SFTPFilePollerSourceProtocolHandlerDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, _ *datasource.ConfigureResponse) {
...
func (d *SFTPFilePollerSourceProtocolHandlerDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	d.pData.Mu.Lock()
	defer d.pData.Mu.Unlock()

	var data SFTPFilePollerSourceProtocolHandlerList
	resp.Diagnostics.Append(req.Config.Get(ctx, &data)...)
	if resp.Diagnostics.HasError() {
		return
	}
	o := models.SFTPFilePollerSourceProtocolHandler{
	...
	l := []models.SFTPFilePollerSourceProtocolHandler{}
	if value := res.Get(`SFTPFilePollerSourceProtocolHandler`); value.Exists() {
		for _, v := range value.Array() {
			item := models.SFTPFilePollerSourceProtocolHandler{}
			item.FromBody(ctx, "", v)
			if !item.IsNull() {
				l = append(l, item)
			}
	...
	if len(l) > 0 {
		...
		data.Result, diag = types.ListValueFrom(ctx, types.ObjectType{AttrTypes: models.SFTPFilePollerSourceProtocolHandlerObjectType}, l)
		resp.Diagnostics.Append(diag...)
		if resp.Diagnostics.HasError() {
			return
		}
	} else {
		data.Result = types.ListNull(types.ObjectType{AttrTypes: models.SFTPFilePollerSourceProtocolHandlerObjectType})
	}
