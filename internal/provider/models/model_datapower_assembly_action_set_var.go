...
type AssemblyActionSetVar struct {
...
var AssemblyActionSetVarObjectType = map[string]attr.Type{
	"provider_target":    types.StringType,
	"id":                 types.StringType,
	"app_domain":         types.StringType,
	"variable":           types.ListType{ElemType: types.ObjectType{AttrTypes: DmAssemblyActionSetVarObjectType}},
...
func (data AssemblyActionSetVar) GetPath() string {
	rest_path := "/mgmt/config/{domain}/AssemblyActionSetVar"
	rest_path = strings.ReplaceAll(rest_path, "{domain}", url.QueryEscape(data.AppDomain.ValueString()))
	return rest_path
}

func (data AssemblyActionSetVar) IsNull() bool {
	if !data.Id.IsNull() {
		return false
	}
...
func (data AssemblyActionSetVar) ToBody(ctx context.Context, pathRoot string) string {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
	body := ""
	...
	if !data.Variable.IsNull() {
		...
		if len(dataValues) > 0 {
			...
			body, _ = sjson.SetRaw(body, pathRoot+`Variable`, "[]")
		}
	} else {
		body, _ = sjson.SetRaw(body, pathRoot+`Variable`, "[]")
	}
...
func (data *AssemblyActionSetVar) FromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
	...
	if value := res.Get(pathRoot + `Variable`); value.Exists() {
		l := []DmAssemblyActionSetVar{}
		if value := res.Get(`Variable`); value.Exists() {
		...
		if len(l) > 0 {
			data.Variable, _ = types.ListValueFrom(ctx, types.ObjectType{AttrTypes: DmAssemblyActionSetVarObjectType}, l)
		} else {
			data.Variable = types.ListNull(types.ObjectType{AttrTypes: DmAssemblyActionSetVarObjectType})
		}
	} else {
		data.Variable = types.ListNull(types.ObjectType{AttrTypes: DmAssemblyActionSetVarObjectType})
	}
...
func (data *AssemblyActionSetVar) UpdateFromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
