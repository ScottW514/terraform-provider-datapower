...
type AssemblyActionWSDL struct {
...
var AssemblyActionWSDLObjectType = map[string]attr.Type{
	"id":                 types.StringType,
...
func (data AssemblyActionWSDL) GetPath() string {
	rest_path := "/mgmt/config/{domain}/AssemblyActionWSDL"
	rest_path = strings.ReplaceAll(rest_path, "{name}", url.QueryEscape(data.Id.ValueString()))
	rest_path = strings.ReplaceAll(rest_path, "{domain}", url.QueryEscape(data.AppDomain.ValueString()))
	return rest_path
}

func (data AssemblyActionWSDL) IsNull() bool {
	if !data.Id.IsNull() {
		return false
	}
...
func (data AssemblyActionWSDL) ToBody(ctx context.Context, pathRoot string) string {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
	body := ""
...
func (data *AssemblyActionWSDL) FromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
...
func (data *AssemblyActionWSDL) UpdateFromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
