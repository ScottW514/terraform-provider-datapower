...
var APIRoutingObjectType = map[string]attr.Type{
	"id":                 types.StringType,
...
func (data APIRouting) GetPath() string {
	rest_path := "/mgmt/config/{domain}/APIRouting"
	rest_path = strings.ReplaceAll(rest_path, "{name}", url.QueryEscape(data.Id.ValueString()))
	rest_path = strings.ReplaceAll(rest_path, "{domain}", url.QueryEscape(data.AppDomain.ValueString()))
	return rest_path
}

func (data APIRouting) IsNull() bool {
	if !data.Id.IsNull() {
		return false
	}
...
func (data APIRouting) ToBody(ctx context.Context, pathRoot string) string {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
	body := ""
...
func (data *APIRouting) FromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
...
func (data *APIRouting) UpdateFromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
