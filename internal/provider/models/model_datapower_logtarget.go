...
var LogTargetObjectType = map[string]attr.Type{
	...
	"remote_password":        types.StringType,
...
func (data LogTarget) ToBody(ctx context.Context, pathRoot string) string {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
	body := ""
	...
	if !data.LogEvents.IsNull() {
		var values []DmLogEvent
		data.LogEvents.ElementsAs(ctx, &values, false)
	...
	if !data.RemotePassword.IsNull() {
		body, _ = sjson.Set(body, pathRoot+`RemotePassword`, data.RemotePassword.ValueString())
	}
	if !data.RemoteDirectory.IsNull() {
	...
	if !data.LogEventCode.IsNull() {
		var values []string
		data.LogEventCode.ElementsAs(ctx, &values, false)
		for _, val := range values {
	...
	if !data.LogEventFilter.IsNull() {
		var values []string
		data.LogEventFilter.ElementsAs(ctx, &values, false)
		for _, val := range values {
			body, _ = sjson.Set(body, pathRoot+`LogEventFilter`+".-1", map[string]string{"value": val})
		}
	}
	if !data.LogObjects.IsNull() {
		var values []DmLogObject
		data.LogObjects.ElementsAs(ctx, &values, false)
		for _, val := range values {
			body, _ = sjson.SetRaw(body, pathRoot+`LogObjects`+".-1", val.ToBody(ctx, ""))
		}
	}
	if !data.LogIpFilter.IsNull() {
		var values []DmLogIPFilter
		data.LogIpFilter.ElementsAs(ctx, &values, false)
		for _, val := range values {
			body, _ = sjson.SetRaw(body, pathRoot+`LogIPFilter`+".-1", val.ToBody(ctx, ""))
		}
	}
	if !data.LogTriggers.IsNull() {
		var values []DmLogTrigger
		data.LogTriggers.ElementsAs(ctx, &values, false)
		for _, val := range values {
...
func (data *LogTarget) FromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	...
	if value := res.Get(pathRoot + `RemotePassword`); value.Exists() && tfutils.ParseStringFromGJSON(value).ValueString() != "" {
		data.RemotePassword = tfutils.ParseStringFromGJSON(value)
	} else {
		data.RemotePassword = types.StringNull()
	}
...
func (data *LogTarget) UpdateFromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	...
	if value := res.Get(pathRoot + `RemotePassword`); value.Exists() && !data.RemotePassword.IsNull() {
		data.RemotePassword = tfutils.ParseStringFromGJSON(value)
	} else {
		data.RemotePassword = types.StringNull()
	}
