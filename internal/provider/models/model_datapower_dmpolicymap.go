...
type DmPolicyMap struct {
	Match types.String `tfsdk:"match"`
	Rule  types.String `tfsdk:"rule"`
}

var DmPolicyMapObjectType = map[string]attr.Type{
	"match": types.StringType,
	"rule":  types.StringType,
}
var DmPolicyMapObjectDefault = map[string]attr.Value{
...
var DmPolicyMapDataSourceSchema = DataSourceSchema.NestedAttributeObject{
	Attributes: map[string]DataSourceSchema.Attribute{
		...
		"rule": DataSourceSchema.StringAttribute{
			MarkdownDescription: tfutils.NewAttributeDescription("Select the Processing Rule to run.", "", "stylepolicyrule").String,
			Computed:            true,
		},
	},
}
...
func (data DmPolicyMap) ToBody(ctx context.Context, pathRoot string) string {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
	body := ""
...
func (data *DmPolicyMap) FromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
...
func (data *DmPolicyMap) UpdateFromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
