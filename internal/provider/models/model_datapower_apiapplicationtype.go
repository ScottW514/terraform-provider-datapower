...
func (data APIApplicationType) GetPath() string {
	rest_path := "/mgmt/config/{domain}/APIApplicationType"
	rest_path = strings.ReplaceAll(rest_path, "{name}", url.QueryEscape(data.Id.ValueString()))
	rest_path = strings.ReplaceAll(rest_path, "{domain}", url.QueryEscape(data.AppDomain.ValueString()))
	return rest_path
}

func (data APIApplicationType) IsNull() bool {
	if !data.Id.IsNull() {
		return false
	}
...
func (data APIApplicationType) ToBody(ctx context.Context, pathRoot string) string {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
	body := ""
	...
	if !data.SupportedOAuthTypes.IsNull() {
		var values []DmOAuthType
		data.SupportedOAuthTypes.ElementsAs(ctx, &values, false)
...
func (data *APIApplicationType) FromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
	...
	if value := res.Get(pathRoot + `SupportedOAuthTypes`); value.Exists() {
		l := []DmOAuthType{}
		if value := res.Get(`SupportedOAuthTypes`); value.Exists() {
...
func (data *APIApplicationType) UpdateFromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
