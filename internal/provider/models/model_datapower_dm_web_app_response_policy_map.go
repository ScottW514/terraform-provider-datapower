...
type DmWebAppResponsePolicyMap struct {
	Match types.String `tfsdk:"match"`
	Rule  types.String `tfsdk:"rule"`
}

var DmWebAppResponsePolicyMapObjectType = map[string]attr.Type{
	"match": types.StringType,
	"rule":  types.StringType,
}
var DmWebAppResponsePolicyMapObjectDefault = map[string]attr.Value{
	...
	var DmWebAppResponsePolicyMapDataSourceSchema = DataSourceSchema.NestedAttributeObject{
		Attributes: map[string]DataSourceSchema.Attribute{
			...
			"rule": DataSourceSchema.StringAttribute{
				MarkdownDescription: "Select the Web Response Profile to run.",
				Computed:            true,
			},
		},
	}
...
func (data DmWebAppResponsePolicyMap) ToBody(ctx context.Context, pathRoot string) string {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
	body := ""
...
func (data *DmWebAppResponsePolicyMap) FromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
...
func (data *DmWebAppResponsePolicyMap) UpdateFromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
