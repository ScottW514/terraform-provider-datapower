...
var APIRateLimitObjectType = map[string]attr.Type{
	"id":               types.StringType,
...
func (data APIRateLimit) GetPath() string {
	rest_path := "/mgmt/config/{domain}/APIRateLimit"
	rest_path = strings.ReplaceAll(rest_path, "{name}", url.QueryEscape(data.Id.ValueString()))
	rest_path = strings.ReplaceAll(rest_path, "{domain}", url.QueryEscape(data.AppDomain.ValueString()))
	return rest_path
}

func (data APIRateLimit) IsNull() bool {
	if !data.Id.IsNull() {
		return false
	}
...
func (data APIRateLimit) ToBody(ctx context.Context, pathRoot string) string {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
	body := ""
...
func (data *APIRateLimit) FromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
...
func (data *APIRateLimit) UpdateFromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
