...
func (data AssemblyLogicSwitch) GetPath() string {
	rest_path := "/mgmt/config/{domain}/AssemblyLogicSwitch"
	rest_path = strings.ReplaceAll(rest_path, "{domain}", url.QueryEscape(data.AppDomain.ValueString()))
	return rest_path
}

func (data AssemblyLogicSwitch) IsNull() bool {
	if !data.Id.IsNull() {
		return false
	}
...
func (data AssemblyLogicSwitch) ToBody(ctx context.Context, pathRoot string) string {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
	body := ""
...
func (data *AssemblyLogicSwitch) FromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
	...
	if value := res.Get(pathRoot + `Case`); value.Exists() {
		...
		if len(l) > 0 {
			data.Case, _ = types.ListValueFrom(ctx, types.ObjectType{AttrTypes: DmAssemblyLogicExecuteObjectType}, l)
		} else {
			data.Case = types.ListNull(types.ObjectType{AttrTypes: DmAssemblyLogicExecuteObjectType})
		}
	} else {
		data.Case = types.ListNull(types.ObjectType{AttrTypes: DmAssemblyLogicExecuteObjectType})
	}
...
func (data *AssemblyLogicSwitch) UpdateFromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
