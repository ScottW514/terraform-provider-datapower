...
func (data DeploymentPolicyParametersBinding) ToBody(ctx context.Context, pathRoot string) string {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
	body := ""
	...
	if !data.DeploymentPolicyParameter.IsNull() {
		var values []DmDeploymentPolicyParameter
		data.DeploymentPolicyParameter.ElementsAs(ctx, &values, false)
...
func (data *DeploymentPolicyParametersBinding) FromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
	...
	if value := res.Get(pathRoot + `DeploymentPolicyParameter`); value.Exists() {
		l := []DmDeploymentPolicyParameter{}
		if value := res.Get(`DeploymentPolicyParameter`); value.Exists() {
		...
		if len(l) > 0 {
			data.DeploymentPolicyParameter, _ = types.ListValueFrom(ctx, types.ObjectType{AttrTypes: DmDeploymentPolicyParameterObjectType}, l)
		} else {
			data.DeploymentPolicyParameter = types.ListNull(types.ObjectType{AttrTypes: DmDeploymentPolicyParameterObjectType})
		}
	} else {
		data.DeploymentPolicyParameter = types.ListNull(types.ObjectType{AttrTypes: DmDeploymentPolicyParameterObjectType})
	}
...
func (data *DeploymentPolicyParametersBinding) UpdateFromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
