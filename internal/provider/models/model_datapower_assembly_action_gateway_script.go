...
func (data AssemblyActionGatewayScript) GetPath() string {
	rest_path := "/mgmt/config/{domain}/AssemblyActionGatewayScript"
	rest_path = strings.ReplaceAll(rest_path, "{domain}", url.QueryEscape(data.AppDomain.ValueString()))
	return rest_path
}

func (data AssemblyActionGatewayScript) IsNull() bool {
	if !data.Id.IsNull() {
		return false
	}
...
func (data AssemblyActionGatewayScript) ToBody(ctx context.Context, pathRoot string) string {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
	body := ""
...
func (data *AssemblyActionGatewayScript) FromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
...
func (data *AssemblyActionGatewayScript) UpdateFromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
