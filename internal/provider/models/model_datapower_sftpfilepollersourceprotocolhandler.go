...
type SFTPFilePollerSourceProtocolHandler struct {
...
func (data SFTPFilePollerSourceProtocolHandler) GetPath() string {
	rest_path := "/mgmt/config/{domain}/SFTPFilePollerSourceProtocolHandler"
	rest_path = strings.ReplaceAll(rest_path, "{name}", url.QueryEscape(data.Id.ValueString()))
	rest_path = strings.ReplaceAll(rest_path, "{domain}", url.QueryEscape(data.AppDomain.ValueString()))
	return rest_path
}

func (data SFTPFilePollerSourceProtocolHandler) IsNull() bool {
	if !data.Id.IsNull() {
		return false
	}
...
func (data SFTPFilePollerSourceProtocolHandler) ToBody(ctx context.Context, pathRoot string) string {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
	body := ""
...
func (data *SFTPFilePollerSourceProtocolHandler) FromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
...
func (data *SFTPFilePollerSourceProtocolHandler) UpdateFromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
