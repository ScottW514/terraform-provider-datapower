...
type FTPFilePollerSourceProtocolHandler struct {
...
func (data FTPFilePollerSourceProtocolHandler) IsNull() bool {
	if !data.Id.IsNull() {
		return false
	}
...
func (data FTPFilePollerSourceProtocolHandler) ToBody(ctx context.Context, pathRoot string) string {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
	body := ""
...
func (data *FTPFilePollerSourceProtocolHandler) FromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
...
func (data *FTPFilePollerSourceProtocolHandler) UpdateFromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
