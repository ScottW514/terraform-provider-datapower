...
type AssemblyActionFunctionCall struct {
	Id              types.String      `tfsdk:"id"`
	AppDomain       types.String      `tfsdk:"app_domain"`
	FunctionCall    types.String      `tfsdk:"function_call"`
...
var AssemblyActionFunctionCallObjectType = map[string]attr.Type{
	"id":               types.StringType,
	"app_domain":       types.StringType,
	"function_call":    types.StringType,
	"parameter":        types.ListType{ElemType: types.ObjectType{AttrTypes: DmAssemblyActionFunctionCallParameterObjectType}},
...
func (data AssemblyActionFunctionCall) GetPath() string {
	rest_path := "/mgmt/config/{domain}/AssemblyActionFunctionCall"
	rest_path = strings.ReplaceAll(rest_path, "{name}", url.QueryEscape(data.Id.ValueString()))
	rest_path = strings.ReplaceAll(rest_path, "{domain}", url.QueryEscape(data.AppDomain.ValueString()))
	return rest_path
}

func (data AssemblyActionFunctionCall) IsNull() bool {
	if !data.Id.IsNull() {
		return false
	}
...
func (data AssemblyActionFunctionCall) ToBody(ctx context.Context, pathRoot string) string {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
	body := ""
	...
	if !data.FunctionCall.IsNull() {
		body, _ = sjson.Set(body, pathRoot+`FunctionCall`, data.FunctionCall.ValueString())
	}
	if !data.Parameter.IsNull() {
		var values []DmAssemblyActionFunctionCallParameter
...
func (data *AssemblyActionFunctionCall) FromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
	...
	if value := res.Get(pathRoot + `FunctionCall`); value.Exists() && tfutils.ParseStringFromGJSON(value).ValueString() != "" {
		data.FunctionCall = tfutils.ParseStringFromGJSON(value)
	} else {
		data.FunctionCall = types.StringNull()
	}
	if value := res.Get(pathRoot + `Parameter`); value.Exists() {
		...
		if len(l) > 0 {
			data.Parameter, _ = types.ListValueFrom(ctx, types.ObjectType{AttrTypes: DmAssemblyActionFunctionCallParameterObjectType}, l)
		} else {
			data.Parameter = types.ListNull(types.ObjectType{AttrTypes: DmAssemblyActionFunctionCallParameterObjectType})
		}
	} else {
		data.Parameter = types.ListNull(types.ObjectType{AttrTypes: DmAssemblyActionFunctionCallParameterObjectType})
	}
...
func (data *AssemblyActionFunctionCall) UpdateFromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
	...
	if value := res.Get(pathRoot + `FunctionCall`); value.Exists() && !data.FunctionCall.IsNull() {
		data.FunctionCall = tfutils.ParseStringFromGJSON(value)
	} else {
		data.FunctionCall = types.StringNull()
	}
