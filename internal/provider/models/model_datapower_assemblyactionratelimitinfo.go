...
type AssemblyActionRateLimitInfo struct {
...
var AssemblyActionRateLimitInfoObjectType = map[string]attr.Type{
	"id":               types.StringType,
...
func (data AssemblyActionRateLimitInfo) GetPath() string {
	rest_path := "/mgmt/config/{domain}/AssemblyActionRateLimitInfo"
	rest_path = strings.ReplaceAll(rest_path, "{name}", url.QueryEscape(data.Id.ValueString()))
	rest_path = strings.ReplaceAll(rest_path, "{domain}", url.QueryEscape(data.AppDomain.ValueString()))
	return rest_path
}

func (data AssemblyActionRateLimitInfo) IsNull() bool {
	if !data.Id.IsNull() {
		return false
	}
...
func (data AssemblyActionRateLimitInfo) ToBody(ctx context.Context, pathRoot string) string {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
	body := ""
...
func (data *AssemblyActionRateLimitInfo) FromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
...
func (data *AssemblyActionRateLimitInfo) UpdateFromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
