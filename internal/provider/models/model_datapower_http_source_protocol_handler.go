...
type HTTPSourceProtocolHandler struct {
...
func (data HTTPSourceProtocolHandler) GetPath() string {
	rest_path := "/mgmt/config/{domain}/HTTPSourceProtocolHandler"
	rest_path = strings.ReplaceAll(rest_path, "{domain}", url.QueryEscape(data.AppDomain.ValueString()))
	return rest_path
}

func (data HTTPSourceProtocolHandler) IsNull() bool {
	if !data.Id.IsNull() {
		return false
	}
...
func (data HTTPSourceProtocolHandler) ToBody(ctx context.Context, pathRoot string) string {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
	body := ""
...
func (data *HTTPSourceProtocolHandler) FromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
...
func (data *HTTPSourceProtocolHandler) UpdateFromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
