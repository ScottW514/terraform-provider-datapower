...
var AssemblyActionJson2XmlObjectType = map[string]attr.Type{
	...
	"root_element_name":          types.StringType,
	"always_output_root_element": types.BoolType,
	"unnamed_element_name":       types.StringType,
...
func (data AssemblyActionJson2Xml) GetPath() string {
	rest_path := "/mgmt/config/{domain}/AssemblyActionJson2Xml"
	rest_path = strings.ReplaceAll(rest_path, "{name}", url.QueryEscape(data.Id.ValueString()))
	rest_path = strings.ReplaceAll(rest_path, "{domain}", url.QueryEscape(data.AppDomain.ValueString()))
	return rest_path
}

func (data AssemblyActionJson2Xml) IsNull() bool {
	if !data.Id.IsNull() {
		return false
	}
	...
	if !data.RootElementName.IsNull() {
		return false
	}
	if !data.AlwaysOutputRootElement.IsNull() {
		return false
	}
	if !data.UnnamedElementName.IsNull() {
		return false
	}
...
func (data AssemblyActionJson2Xml) ToBody(ctx context.Context, pathRoot string) string {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
	body := ""
	...
	if !data.RootElementName.IsNull() {
		body, _ = sjson.Set(body, pathRoot+`RootElementName`, data.RootElementName.ValueString())
	}
	if !data.AlwaysOutputRootElement.IsNull() {
		body, _ = sjson.Set(body, pathRoot+`AlwaysOutputRootElement`, tfutils.StringFromBool(data.AlwaysOutputRootElement, ""))
	}
	if !data.UnnamedElementName.IsNull() {
		body, _ = sjson.Set(body, pathRoot+`UnnamedElementName`, data.UnnamedElementName.ValueString())
	}
	if !data.UserSummary.IsNull() {
...
func (data *AssemblyActionJson2Xml) FromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
	...
	if value := res.Get(pathRoot + `RootElementName`); value.Exists() && tfutils.ParseStringFromGJSON(value).ValueString() != "" {
		data.RootElementName = tfutils.ParseStringFromGJSON(value)
	} else {
		data.RootElementName = types.StringValue("json")
	}
	if value := res.Get(pathRoot + `AlwaysOutputRootElement`); value.Exists() {
		data.AlwaysOutputRootElement = tfutils.BoolFromString(value.String())
	} else {
		data.AlwaysOutputRootElement = types.BoolNull()
	}
	if value := res.Get(pathRoot + `UnnamedElementName`); value.Exists() && tfutils.ParseStringFromGJSON(value).ValueString() != "" {
		data.UnnamedElementName = tfutils.ParseStringFromGJSON(value)
	} else {
		data.UnnamedElementName = types.StringValue("element")
	}
...
func (data *AssemblyActionJson2Xml) UpdateFromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
	...
	if value := res.Get(pathRoot + `RootElementName`); value.Exists() && !data.RootElementName.IsNull() {
		data.RootElementName = tfutils.ParseStringFromGJSON(value)
	} else if data.RootElementName.ValueString() != "json" {
		data.RootElementName = types.StringNull()
	}
	if value := res.Get(pathRoot + `AlwaysOutputRootElement`); value.Exists() && !data.AlwaysOutputRootElement.IsNull() {
		data.AlwaysOutputRootElement = tfutils.BoolFromString(value.String())
	} else if data.AlwaysOutputRootElement.ValueBool() {
		data.AlwaysOutputRootElement = types.BoolNull()
	}
	if value := res.Get(pathRoot + `UnnamedElementName`); value.Exists() && !data.UnnamedElementName.IsNull() {
		data.UnnamedElementName = tfutils.ParseStringFromGJSON(value)
	} else if data.UnnamedElementName.ValueString() != "element" {
		data.UnnamedElementName = types.StringNull()
	}
