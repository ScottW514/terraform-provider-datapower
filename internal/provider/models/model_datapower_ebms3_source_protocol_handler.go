...
type EBMS3SourceProtocolHandler struct {
...
var EBMS3SourceProtocolHandlerSSLSNIServerIgnoreVal = validators.Evaluation{
	Evaluation:  "property-value-not-in-list",
	Attribute:   "ssl_server_config_type",
	AttrType:    "String",
	AttrDefault: "server",
...
func (data EBMS3SourceProtocolHandler) IsNull() bool {
	if !data.Id.IsNull() {
		return false
	}
...
func (data EBMS3SourceProtocolHandler) ToBody(ctx context.Context, pathRoot string) string {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
	body := ""
...
func (data *EBMS3SourceProtocolHandler) FromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
...
func (data *EBMS3SourceProtocolHandler) UpdateFromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
