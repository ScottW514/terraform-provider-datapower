...
func (data GWSRemoteDebug) GetPath() string {
	rest_path := "/mgmt/config/default/GWSRemoteDebug/GWSRemoteDebug"
	return rest_path
}

func (data GWSRemoteDebug) IsNull() bool {
...
func (data *GWSRemoteDebug) FromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
...
func (data *GWSRemoteDebug) UpdateFromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
	...
	if value := res.Get(pathRoot + `LocalPort`); value.Exists() && !data.LocalPort.IsNull() {
		data.LocalPort = types.Int64Value(value.Int())
	} else if data.LocalPort.ValueInt64() != 9229 {
