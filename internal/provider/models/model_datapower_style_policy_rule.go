...
var StylePolicyRuleNonXMLProcessingIgnoreVal = validators.Evaluation{
...
func (data StylePolicyRule) GetPath() string {
	rest_path := "/mgmt/config/{domain}/StylePolicyRule"
	rest_path = strings.ReplaceAll(rest_path, "{domain}", url.QueryEscape(data.AppDomain.ValueString()))
	return rest_path
}

func (data StylePolicyRule) IsNull() bool {
	if !data.Id.IsNull() {
		return false
	}
...
func (data StylePolicyRule) ToBody(ctx context.Context, pathRoot string) string {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
	body := ""
...
func (data *StylePolicyRule) FromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
...
func (data *StylePolicyRule) UpdateFromBody(ctx context.Context, pathRoot string, res gjson.Result) {
	if pathRoot != "" {
		pathRoot = pathRoot + "."
	}
